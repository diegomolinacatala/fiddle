      ...
      <div style={{ width: "min(430px, 94vw)" }}>
        ...
        {manual && (
          <form onSubmit={abrir} style={{ display: "flex", gap: 8, marginTop: 10 }}>
            <input
              ...
              placeholder="serial o URL del pase"
              style={input}
            />
            <button type="submit" style={go}>Abrir</button>
          </form>
        )}

        <div style={{ marginTop: 24, fontSize: 12, opacity: 0.45, textTransform: "uppercase", letterSpacing: 1 }}>
          Clientes recientes ({clientes.length})
        </div>
        {clientes.slice(0, 12).map((c) => (
          <a key={c.serial} href={`/w/${c.serial}`} style={row}>
            ...
            <span style={{ opacity: 0.8 }}>{c.sellos} sellos · {c.premios || 0} 🎁</span>
          </a>
        ))}
        {clientes.length === 0 && (
          <p style={{ opacity: 0.5, fontSize: 14 }}>Aún no hay clientes. Emite uno desde el manager o /api/tap.</p>
        )}
      </div>
    </main>
  );
