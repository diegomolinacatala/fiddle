...
import { nivelDe } from "@/lib/apple/pase";
      ...
      <div style={{ width: "min(360px, 92vw)", textAlign: "center" }}>
        ...
        {(appleUrl || googleUrl) && (
          <div style={{ display: "grid", gap: 10, marginTop: 16 }}>
            {/* Para publicar, sustituir por el badge oficial "Add to Apple Wallet" (Apple Wallet Identity Guidelines). */}
            {appleUrl && <a href={appleUrl} style={botonWallet("#000", "#fff")}> Añadir a Apple Wallet</a>}
            {googleUrl && <a href={googleUrl} style={botonWallet("#fff", "#000")}>Guardar en Google Wallet</a>}
          </div>
        )}
...
const botonWallet = (fondo, texto) => ({
  display: "block", padding: "0.85rem 1rem", borderRadius: 12, background: fondo, color: texto,
  fontWeight: 600, fontSize: 16, textDecoration: "none", border: "1px solid rgba(0,0,0,.2)",
});
...
function Barber({ n, c }) {
  ...
  const nivel = nivelDe(c.premios || 0);
  ...
  return (
    <div style={{ position: "relative", overflow: "hidden", background: n.tema.cardBg, color: n.tema.ink, borderRadius: 20, padding: 22, border: "1px solid rgba(201,162,75,.35)", textAlign: "left" }}>
      ...
      <div style={{ paddingLeft: 8 }}><Titular c={c} /></div>
...
function Titular({ c }) {
  return c.nombre ? <div style={{ marginTop: 6, fontSize: 14, opacity: 0.8 }}>👤 {c.nombre}</div> : null;
}
